...
root_folder = 'pulse/data/top/transaction/country/india/state'  # Your main directory
all_data = []
...
toptn={'States':[], 'Yr':[],'Quater':[],'Entity_Name':[], 'Transacion_count':[], 'Transacion_amount':[], 'Entity_type':[]}

# Corrected path for top transaction state data
                        ...
                        for entity_type in ['states', 'districts', 'pincodes']:
                            if entity_type in D['data'] and D['data'][entity_type] is not None:
                                for entity_data in D['data'][entity_type]:
                                    ...
                                    toptn['Entity_Name'].append(Name)
                                    toptn['Transacion_count'].append(count)
                                    toptn['Transacion_amount'].append(amount)
                                    toptn['States'].append(state)
                                    toptn['Yr'].append(year)
                                    toptn['Quater'].append(int(qtr_file.strip('.json')))
                                    toptn['Entity_type'].append(entity_type)

#Succesfully created a dataframe
Top_trans=pd.DataFrame(toptn)
print(Top_trans)
#<-------------------------------------------------------------------------------------------->
...
Top_trans.to_sql('toptransaction', con=engine, if_exists='replace', index=False)
